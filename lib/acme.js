...
class AcmeProtocol {
   ...
   *
   * @param  {string}         url URL for this registration
   ...
   * @return {Promise<registration>} the new registration
   */
  updateRegistration(url, registration) {
    if (!url) {
      throw new TypeError('No registration URL provided');
    }
    registration.resource = 'reg';
    return this._post(url, registration);
  }
