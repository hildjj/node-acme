...
var questions = {
  ...
  readyToValidate: [{
    type: 'input',
    name: 'noop',
    message: 'Press enter to when you are ready to proceed',
  }],
...
function sendAgreement(answers) {
  ...
  if (state.termsRequired && !state.termsAgreed) {
    console.log('Sorry, cannot proceed if you do not agree.');
    process.exit(1);
  }
  ...
  post(state.registrationURL, state.registration,
    function(err, resp, body) {
      if (err || Math.floor(resp.statusCode / 100) != 2) {
        ...
        console.log('Could not POST agreement back to server, aborting.');
        process.exit(1);
      } else {
        inquirer.prompt(questions.domain, getChallenges);
      }
...
function getReadyToValidate(err, resp, body) {
  ...
  if (simpleHttps.length == 0) {
    console.log('The server did not offer any challenges we can handle.');
    return;
  }
