...
class ACMEServer {
  ...
  constructor(options) {
    ...
    this.transport.app.get('/:type/:id', (req, res) => this.fetch(req, res));
    this.transport.app.get(DIRECTORY_TEMPLATE['directory'], (req, res) => this.directory(req, res));
    this.transport.app.post(DIRECTORY_TEMPLATE['new-reg'], (req, res) => this.newReg(req, res));
    // TODO others
  }
  ...
  newReg(req, res) {
    ...
    jwk.thumbprint()
      .then(tpBuffer => {
        let thumbprint = tpBuffer.toString('hex');
        ...
        if (this.terms) {
          res.links({'terms-of-service': this.termsURL});
        }
