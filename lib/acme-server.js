...
class Authorization {
  constructor(server, thumbprint, name, scope) {
    ...
    this.expires.setTime(this.expires.getTime() + offset);
    ...
    if (server.policy.challenges.auto) {
      ...
      this.challenges.push({
        type:   'bogus',
        status: 'invalid',
        update: () => { this.status = 'valid'; },
        toJSON: () => { return {type: this.type, status: this.status}; }
      });
    }
    this.update();
  }
  ...
  update() {
    this.challenges = this.challengeObj.map(x => x.toJSON());
...
class ACMEServer {
  ...
  updateAuthz(req, res) {
    ...
    let authz = this.db.get(Authorization.type, req.id);
    ...
    // async ultimately.
    authz.challenges[index].update(res.payload)
      .then(() => { authz.update(); })
      .then(() => { this.db.updateAppsFor(authz); })
      .then(() => {
        res.status(200);
