...
var dns     = require('native-dns');     // https://github.com/tjfontaine/node-dns
...
/**
 * Create a server to respond to the 'dns-01' challenge.  All validation
 * servers have listen(port) and close() methods.
 *
 * XXX(rlb): Should turn this into a class.
 *
 ...
 * @throws {TypeError} Invalid challenge
 */
module.exports.DNS01Server = function(domain, challenge) {
  if (!util.fieldsPresent(CHALLENGE_FIELDS, challenge) ||
  ...
  server.on('request', function dns01_serve(request, response) {
  ...
  this.listen = function dns01_listen(port) {
    return Promise.resolve(server.serve(port));
  };

  this.close = function dns01_close() {
    return server.close();
  };
