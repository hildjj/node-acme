...
class CLI {
  ...
  _authzOpts(yargs) {
    ...
    this.argv = utils.extend(this.argv,
      ...
      .options({
        ...
        domain: {
          description: 'The domain to authorize',
          string:      true,
          requiresArg: true,
          alias:       'd'
        }
  ...
  exec() {
    ...
    .then(() => {
      ...
      switch (this.cmd) {
        case 'init':   return this.generate();
        case 'reg':    return this.register();
        case 'update': return this.update();
        case 'authz':  return this.authz();
        default:
  ...
  update() {
    ...
    return protocol.get(this.config.registrationURI)
    .then(reg => {
  ...
  authz() {
    return this._proto()
    .newAuthorization(this.config.domain);
  }
}
